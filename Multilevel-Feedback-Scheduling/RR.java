public class RR {
  ...
  public RR(int[] process, int[] arrivalTime, int[] burstTime,  int[] priority, int quantum, int queue){
              ...
              if (res_a[j] < res_a[i]) {
                if (res_b[j] > 0) {
                  ...
                  if (res_b[j] > quantum) {
                    ...
                    waitingTime[j] = t - burstTime[j] - arrivalTime[j];
                    seq += "->" + process[j];
                    res_b[j] = 0;
                  }
                }
    ...
    for(int i = 0; i < size; i++){
      turnaroundTime[i] = waitingTime[i] + arrivalTime[i];
      if(queue == 0)
      responseTime[i] = turnaroundTime[i] - waitingTime[i];
      waitingTime[i] = Math.abs(burstTime[i] - responseTime[i]);
      ...
      try{
        ...
        for(int k = 0; k < process.length; k++){
          if(res_b[k] != 0){
            ...
            MLFQFrame.burstTime[nextQueue][temporary] = res_b[k];
