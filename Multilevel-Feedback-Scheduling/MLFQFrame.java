...
public class MLFQFrame extends JFrame implements ActionListener {
	...
	private JButton processClearButton = new JButton("CLEAR");
	...
	public void createTable() {
		...
		processClearButton.setEnabled(false);
		...
		processNumPanel.add(processClearButton);
		processNumPanel.add(randomButton);
		...
		processClearButton.addActionListener(this);
	...
	public void createGantt() {
		...
		ganttPanel.setBorder(BorderFactory.createLineBorder(Color.black));
		...
		q1Label.setFont(new Font("Sans_Serif", Font.BOLD, 20));
		q1Label.setBackground(new Color(200, 200, 200));
		q1Label.setBounds(1, 20, 900, 38);
		...
		pane1.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_NEVER);
		pane1.setBounds(0, 60, 940,128);
	...
	public void createAlgo() {
		...
		startButton.setBounds(10, 260, 90, 40);
		startButton.setEnabled(false);

		algoPanel.add(clearButton);
		clearButton.setBounds(110, 260, 90, 40);

		algoPanel.add(stopButton);
		stopButton.setBounds(210, 260, 90, 40);

		algoPanel.add(exitButton);
		exitButton.setBounds(310, 260, 90, 40);
		...
		for(int i = 0; i < numOfQueues; i++){
			...
			if(policyDropbox.getSelectedIndex() == 0){
				if(algorithmDropbox[i].getSelectedIndex() == 5){
					if(quanTextField[i].getText().equals("0")){
						JOptionPane.showMessageDialog(this, "Quantum should not be equal to zero", "Invalid Input", JOptionPane.ERROR_MESSAGE);
						return;
					}else{
						try{
		...
		while(isTrue){
			try{
				if(policyDropbox.getSelectedIndex() == 0){
					...
					if(queues[i] == 0){
						new FCFS(processID[i], arrivalTime[i], burstTime[i], priority[i], i, quantum[i]+1);
					}
					else if(queues[i] == 1){
						new SJF(processID[i], arrivalTime[i], burstTime[i], priority[i], i, quantum[i]+1);
					}
					else if(queues[i] == 2){
						new SRTF(processID[i], arrivalTime[i], burstTime[i], priority[i], i, quantum[i]+1);
					}
					else if(queues[i] == 3){
						new NP(processID[i], arrivalTime[i], burstTime[i], priority[i], i, quantum[i]+1);
					}
					else if(queues[i] == 4){
						new PP(processID[i], arrivalTime[i], burstTime[i], priority[i], i, quantum[i]+1);
					}
					else if(queues[i] == 5){
						try{
							new RR(processID[i], arrivalTime[i], burstTime[i], priority[i], quantum[i], i);
			...
			for(int n = 1; n < tableColumn; n++){
				...
				if(panelHolder[m][n].getText().isEmpty()){
					if(n == 1){
						...
						panelHolder[m][n].setText(""+randomNum);
					}else if (n == 2){
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(event.getSource() == processClearButton){
			resetTable();
		}
